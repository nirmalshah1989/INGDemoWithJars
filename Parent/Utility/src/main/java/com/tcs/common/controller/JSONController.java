...
public class JSONController {
	...
	public @ResponseBody Status getApp1Status() {
		...
		if (isAccessable("http://localhost:8080/WebModule/rest/dependent1/hello")) {
			status.setStatus("Dependent-1 (http://localhost:8080/WebModule/rest/dependent1/hello) is UP");
		} else {
			status.setStatus("Dependent-1 (http://localhost:8080/WebModule/rest/dependent1/hello) is DOWN");
		}
		return status;
	}
	...
	public @ResponseBody Status getApp2Status() {
		...
		Status status = new Status();
		if (isAccessable("http://localhost:8080/WebModule/rest/dependent2/hello")) {
			status.setStatus("Dependent-2 (http://localhost:8080/WebModule/rest/dependent2/hello) is UP");
		} else {
			status.setStatus("Dependent-2 (http://localhost:8080/WebModule/rest/dependent2/hello) is DOWN");
		}
		return status;
	}
